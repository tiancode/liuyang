...
#include <string.h>
/**
 * 32-bit integer monipulation macros (little endian)
 */
...
void md5_finish( md5_context *ctx, unsigned char output[16] )
{
    	...
    	SET_UINT32_LE( low,  msglen, 0 );
    	SET_UINT32_LE( high, msglen, 4 );
    	...
    	SET_UINT32_LE( ctx->state[0], output,  0 );
    	SET_UINT32_LE( ctx->state[1], output,  4 );
    	SET_UINT32_LE( ctx->state[2], output,  8 );
    	SET_UINT32_LE( ctx->state[3], output, 12 );
}
